...
## 5. UI/UX 操作（追補）
- **ノード表示**：ComfyUIライクな**カードUI**を採用。ノードヘッダーにタイトル/タイプ/バージョンを表示し、左右に**入力/出力ポート**を並列配置する。ポートにはデータ型バッジと接続状態のカラーリングを付け、視覚的フィードバックを提供。
- **ポート接続**：出力ポートを**ドラッグ開始→入力ポートへドロップ**で即接続。途中でキャンバスへドロップするとキャンセル。キーボード操作時は従来通りクリック/Enterで接続可能。ドラッグ中はカーソルに追従する**ベジェ曲線プレビュー**を表示し、接続完了後はSVGレイヤー上に常時描画する。
- **接続リスト**：右サイドバーの「接続」カードには最新状態が即時反映され、各行から**接続解除**が行える。新規接続待ち状態では「○○から接続先を選択」のヒントをリアルタイム表示。
...
- **AC-CONN-002**：出力ポートのドラッグ開始→入力ポートにドロップで接続が確立し、キャンバスドロップでキャンセルできる。接続中はカーソル追従のプレビュー曲線が表示され、接続完了後はSVGベジェで描画される。
